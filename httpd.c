...
/**********************************************************************/
void accept_request(int client)//client 已有连接套接口fd
{
	 char buf[1024];
	 int numchars;
	 char method[255];
...
int startup(u_short *port)
{
	 ...
	 //AF表示地址族，PF表示协议族，基本相等，多使用AF，posix使用pf
	 ...
	 */
	 //最初的 BSD socket 实现中，backlog 的上限是5>=已完成连接队列和未完成连接队列
	 if (listen(httpd, 5) < 0) 
	  error_die("listen");
	 return(httpd);
}
...
int main(void)
{
	...
	//pthread_t newthread;
	 ...
	 while (1)
	 {
		  //阻塞等待客户端的连接，从已完成连接队列返回下一个已完成连接（队头）
		  client_sock = accept(server_sock,
		                       (struct sockaddr *)&client_name,
		                       &client_name_len);
		 if (client_sock == -1)//client_sock表示tcp连接
		  	 error_die("accept");
		 printf("server:get a client!\n");
		  //accept_request(client_sock);//origin code
		  pool_add_worker(accept_request, &client_sock);//参数为回调函数
		 //if (pthread_create(&newthread , NULL, accept_request, client_sock) != 0)
		  // 	perror("pthread_create");//origin code
	 }
	 pool_destroy();
	close(server_sock);
